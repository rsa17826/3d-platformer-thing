# +space to enter rotate mode
# ^ sprint
# w|a|s|d move
# space go up
# shift go down
# in rotate mode
# wasdqe to rotate target block
@tool
extends Window
...
func _input(event: InputEvent) -> void:
    ...
    if key and key.pressed and !(Input.is_key_pressed(KEY_SPACE)&&Input.is_key_pressed(KEY_SHIFT)):
        match key.keycode:
...
func _process(delta: float) -> void:
    var speed = 0.5 if Input.is_key_pressed(KEY_CTRL) else 0.1
    
    var forward := -player.global_basis.z
    var right := player.global_basis.x
    if !(Input.is_key_pressed(KEY_SPACE)&&Input.is_key_pressed(KEY_SHIFT)):
